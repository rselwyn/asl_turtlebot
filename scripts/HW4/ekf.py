...
class EkfLocalization(Ekf):
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        V = np.dstack((angle_diff(z_raw[0,...,None], hs[0,None,...]), z_raw[1,...,None]-hs[1,None,...]))

        H = np.array(Hs)
        Q = np.array(Q_raw)

        S = H @ self.Sigma @ H.transpose((0,2,1)) + Q[:,None,...]

        d = (V[...,None,:] @ np.linalg.inv(S) @ V[...,None])[...,0,0]
        
        i = np.nonzero(np.min(d, axis=-1) < self.g**2)[0]
        j = np.argmin(d, axis=-1)[i]
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        g[0:3], Gx[0:3,0:3], Gu[0:3,:] = tb.compute_dynamics(self.x, u, dt)
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx[:,0:3] = tb.transform_line_to_scanner_frame((alpha, r), self.x, self.tf_base_to_camera)

            # First two map lines are assumed fixed so we don't want to propagate
            # any measurement correction to them.
            if j >= 2:
                R = np.array([
                    [np.cos(self.x[2]), -np.sin(self.x[2])],
                    [np.sin(self.x[2]), np.cos(self.x[2])],
                ])
                t_camera_w = R @ self.tf_base_to_camera[0:2] + self.x[0:2]
                r_c = r - (np.cos(alpha)*t_camera_w[0] + np.sin(alpha)*t_camera_w[1])
                Hx[:,idx_j:idx_j+2] = np.array([
                    [1, 0],
                    (-1 if r_c < 0 else 1)*np.array([-(-np.sin(alpha)*t_camera_w[0] + np.cos(alpha)*t_camera_w[1]), 1]),
                ])
            ########## Code ends here ##########
