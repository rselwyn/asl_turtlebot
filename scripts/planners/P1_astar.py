...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        return self.occupancy.is_free(x) and all(a >= b for a, b in zip(x, self.statespace_lo)) and all(a <= b for a, b in zip(x, self.statespace_hi))
    ...
    def get_neighbors(self, x):
        ...
        snap = lambda x: self.resolution*np.round(x/self.resolution)
        ...
        neighbors = [(i[0], i[1]) for i in snap(adjust + np.array(x)) if self.is_free(i)]
    ...
    def solve(self):
        ...
        while len(self.open_set) > 0:
            ...
            est_cost_through_open_set = {k: v for k, v in self.est_cost_through.items() if k in self.open_set}
            x_current = min(est_cost_through_open_set, key=est_cost_through_open_set.get)
